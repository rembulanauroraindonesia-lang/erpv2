...
router = APIRouter(prefix="/app/purchase-order", tags=["htmx-purchase-order"])
...
async def po_index(
    ...
    if search:
        query = query.where(PurchaseOrder.nomor.ilike(f"%{search}%"))
    if status:
    ...
    return templates.TemplateResponse("purchase_order/index.html", {
        "request": request,
        "docs": docs,
        "suppliers": suppliers,
        "total": total,
...
@router.get("/new")
async def po_new(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse("purchase_order/new.html", {
        "request": request,
        "suppliers": suppliers,
        "items": items,
        "active": "purchase_order",
...
@router.get("/{doc_id}")
async def po_detail(request: Request, doc_id: str, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        ...
        .where(PurchaseOrderItem.purchase_order_id == doc_id)
        .order_by(PurchaseOrderItem.urutan)
    )
    ...
    return templates.TemplateResponse("purchase_order/detail.html", {
        "request": request,
        "doc": doc,
        "items": items,
        "supplier": supplier,
