...
router = APIRouter(prefix="/app/proforma-invoice", tags=["htmx-proforma-invoice"])
...
async def pi_index(
    ...
    return templates.TemplateResponse("proforma_invoice/index.html", {
        "request": request, "active": "proforma_invoice",
...
@router.get("/new")
async def pi_create_form(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse("proforma_invoice/new.html", {
        "request": request, "active": "proforma_invoice",
...
@router.get("/{doc_id}")
async def pi_detail(doc_id: str, request: Request, db: AsyncSession = Depends(get_db)):
    doc = await db.get(ProformaInvoice, doc_id)
    if not doc or doc.is_deleted:
        from fastapi.responses import RedirectResponse
        return RedirectResponse("/app/proforma-invoice", 302)
    ...
    return templates.TemplateResponse("proforma_invoice/detail.html", {
        "request": request, "active": "proforma_invoice",
        "doc": doc, "items": items, "customer": customer,
        "item_names": item_names, "company": company, "today": date.today(),
    })
